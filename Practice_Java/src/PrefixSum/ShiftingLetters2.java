...
[starti, endi, directioni]. For every i, shift the characters in s from the index starti to the index endi (inclusive)
...
public class ShiftingLetters2 {
    /**
     * Approach 1 : Using a Prefix sum array, calculate the total movements and shifting and then add the same.
     * Complexity Analysis :
    ...
    public String shiftingLetters(String s, int[][] shifts) {
        ...
        for (int[] sh : shifts) {
            int start = sh[0], end = sh[1], direction = sh[2] == 1 ? 1 : -1;
        ...
        for (int i = 0; i < n; i++) {
            ...
            int currentShift = (s.charAt(i) - 'a' + cumulativeShift) % 26;
            if (currentShift < 0) {
                currentShift += 26;
            }
            result[i] = (char) (currentShift + 'a');
        }
