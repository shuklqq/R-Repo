...
public class MinimumCostForTickets {
    ...
    public int mincostTickets(int[] days, int[] costs) {
        ...
        Arrays.fill(dp, 0);
        int oneCost = costs[0], sevenCost = costs[1], thirtyCost = costs[2];
        ...
        for(int day = 1 ; day <= lastDay ; day++){
            if(day < days[i]){
                ...
                i++;
                dp[day] = Math.min(dp[day - 1] + oneCost, Math.min(dp[Math.max(0 , day -7)] + sevenCost, dp[Math.max(0 , day - 30)] + thirtyCost));
            }
        }
        return dp[lastDay];
    }
